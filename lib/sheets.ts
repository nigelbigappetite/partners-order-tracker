...
// Create a new franchise/location row in Franchisee_Master
export async function createFranchiseEntry(franchiseData: {
  code: string;
  name: string;
  city?: string;
  country?: string;
  ...
  try {
    ...
    console.log('[createFranchiseEntry] Appended new franchise entry to Franchisee_Master:', {
      ...
      activeBrands: franchiseData.activeBrands,
    });
  } catch (error: any) {
    console.error('[createFranchiseEntry] Error creating franchise entry:', error.message || error);
    throw error;
  }
}
  ...
  try {
    ...
    let normalizedInvoices = invoices.map((inv: any, index: number) => {
      ...
      return {
        ...
        sales_invoice_no: (inv.sales_invoice_no || inv['Sales Invoice No'] || inv['Sales Invoice Number'] || inv['Linked Order'] || inv['Order'] || inv.Linked_Order || inv.Sales_Invoice_No || '').toString().trim(),
        supplier: (inv.supplier || inv.Supplier || '').toString().trim(),
        amount: Number(inv.amount ?? inv.Amount ?? inv['Invoice Total'] ?? inv['Total'] ?? inv.invoice_total ?? inv.Invoice_Total ?? 0),
